...
as
  ...
  is
  begin
    update invg_invoice_lines
       set active_yn = 'N'
     where invg_invoice_id = p_invoice_id;

    update invg_invoices
       set active_yn = 'N'
     where invg_invoice_id = p_invoice_id;
  end delete_invoice;
  ...
  is
    l_max_num   number;
    l_prefix    varchar2(50 char) := to_char(sysdate, 'YYYY') || '-';
    l_next_str  varchar2(50 char);
  begin
    select nvl(max(
             to_number(
               regexp_replace(invoice_number, '^.*-', '')
             )
           ), 0)
      into l_max_num
      from invg_invoices
     where active_yn = 'Y'
       and regexp_like(invoice_number, '^[0-9]{4}-\d+$');
    l_next_str := l_prefix || lpad(to_char(l_max_num + 1), 3, '0');
    return l_next_str;
  exception
